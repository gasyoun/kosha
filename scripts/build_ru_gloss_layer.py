...
Also importable: `RuGlosser` (loaded-once join primitive, `.gloss(form_iast, lemma_iast,
lemma_slp1)`) and `compute_family_coverage(glosser)` (this family's per-pack tot/hit
numbers only) are what `build_subhashita_pack.py` (H1312) reuses to fold its own
lines/chunks-schema pack's RU-gloss coverage into ONE merged
reading/RU_GLOSS_COVERAGE.md alongside this family, without re-deriving the join or
duplicating the writer -- run whichever of the two builders last to regenerate the
full merged report.
...
def compute_family_coverage(glosser):
    """(per_pack, uncovered) for the reading/data/*.json family only — the same
    per-pack tot/lemma_hit computation main()'s TSV-writing loop performs, factored
    out so a caller outside this family (e.g. build_subhashita_pack.py, whose pack
    schema is lines/chunks, not sentences/tokens, and so is never picked up by
    pack_files()) can fold its own numbers into ONE coverage report without
    re-deriving this join or duplicating the TSV write. Added H1312."""
...
def write_coverage_report(per_pack, uncovered):
    ...
    `per_pack` may be the reading-pack family alone (this script's own main()) or that
    family merged with a non-family pack's numbers (build_subhashita_pack.py, H1312) —
    either caller regenerates the SAME file, full-table, from live source each time."""
    L = ["# Reading packs — Russian gloss coverage (W-RU-a / H1278, +subhāṣita / H1312)", "",
         "_Auto-generated by `scripts/build_ru_gloss_layer.py` (Opus 4.8 `claude-opus-4-8`) "
         "and folded together with `scripts/build_subhashita_pack.py` (Sonnet 5 "
         "`claude-sonnet-5`, H1312). RU glosses from the SanskritRussian **public "
         "site-tier** subset only (surface/lemma/root glossaries); the restricted "
         "`corpus_lexicon` is not read._", "",
