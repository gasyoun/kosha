...
| PWK | no | not in CDSL — no `pwk` directory in csl-orig/v02 and no `pwk.zip` in any csl-sqlite release (`pwkvn` is a separate dict, not this one). The 151,314 figure is a union `lemmas` headword count (key1 presence only) — no entry/sense text exists to align. |
...
|---|---|---|
| PWG | `de` | closed |
| MW | `en` | open (MW↔Apte↔MD) |
| Apte | `en` | open (MW↔Apte↔MD) |
| MD | `en` | open (MW↔Apte↔MD) |
| ŚKDR | `sa` | closed |
| VCP | `sa` | closed |
...
| senses considered | 75254 |
| senses dropped as `no-gloss` (structural chunks) | 6612 |
| meaning groups | 67330 |
| aligned groups (≥2 dictionaries) | 7003 |
| unaligned singleton senses | 60327 |
| aligned share of groups | 10.4% |
| lemmas with ≥1 aligned group | 1408 |
| lemmas present in all three western dictionaries | 730 |
| ŚKDR senses loaded | 392 |
| VCP senses loaded | 3290 |
| aligned groups touching ŚKDR | 60 |
| ŚKDR↔PWG candidate edges withheld by the root-vs-nominal gate (H5273) | 266 |
...
|---|---:|---:|---:|
| senses considered | 33,763 | 75,254 | +41,491 |
| meaning groups (rows) | 30,470 | 67,330 | +36,860 |
| aligned groups | 2,957 | 7,003 | +4,046 |
| lemmas with ≥1 aligned meaning | 477 | 1,408 | +931 |
| clean three-dictionary `1-1-1` rows | 262 | 16 | -246 |

| failure class | H3744 | this build | Δ |
|---|---:|---:|---:|
| `no-shared-witness` | 17,671 | 37,262 | +19,591 |
| `witness-too-common` | 6,844 | 12,265 | +5,421 |
| `no-gloss` | 3,025 | 6,612 | +3,587 |
| `cross-language-gap` | 1,835 | 4,317 | +2,482 |
| `no-citation-apparatus` | 0 | 3,262 | +3,262 |
| `outranked` | 1,101 | 2,477 | +1,376 |
| `absent-dictionary` | 62 | 492 | +430 |
| `root-vs-nominal` | 0 | 252 | +252 |
...
|---|---:|---|
| `no-shared-witness` | 37262 | the sense cites sources, but no cross-dictionary sense of this lemma cites any of them |
| `witness-too-common` | 12265 | shared witnesses exist but fall below τ on weight — real citation, no discriminating power |
| `no-gloss` | 6612 | a structural chunk (PWG `<div>` carrying only `<lex>m.</lex>`) — excluded before alignment |
| `cross-language-gap` | 4317 | a PWG sense with no `<ls>` at all — the citation bridge does not exist for it, and German→English gloss overlap measures nothing |
...
| `outranked` | 2477 | a qualifying partner existed but preferred a better-scoring sense; each sense takes at most one partner per other dictionary |
| `absent-dictionary` | 492 | the lemma simply has no entry in the other dictionaries |
| `root-vs-nominal` | 252 | H5273 gate: the only edge was a dhātu-marked ŚKDR record against a PWG sense PWG marks non-verbal (`<lex>`, gloss not an infinitive) |
...
```
[aligned   1-1-0-0-0-0 ls        0.50 mbh] PWG: Elephantenzahn, Elfenbein || MW: m. elephant's tusk or ivory
[aligned   1-1-0-0-0-0 ls        0.50 panc] PWG: Pflock in der Wand zum Anhängen von Sachen || MW: a peg in the wall to hang things upon
[unaligned 0-0-0-0-0-1 singleton 0.00 -] VCP: nāgadanta pu0 nāgasya hastino danta ivākāro'styasya ac . gṛhabhittirni  <- no-citation-apparatus
[unaligned 0-0-0-1-0-0 singleton 0.00 -] MD: nāga-danta, m. elephantʼs tooth, ivory; peg or bracket on the wall ; -  <- no-shared-witness
[unaligned 1-0-0-0-0-0 singleton 0.00 -] PWG: f. ( adj. comp. ) N. pr. einer Apsaras  <- no-shared-witness
[unaligned 1-0-0-0-0-0 singleton 0.00 -] PWG: f. ( adj. comp. )  <- cross-language-gap
[unaligned 1-0-0-0-0-0 singleton 0.00 -] PWG: N. einer Pflanze, Tiaridium indicum Lehm  <- no-shared-witness
[unaligned 1-0-0-0-0-0 singleton 0.00 -] PWG: Hure; eine best. Arzeneipflanze  <- no-shared-witness
```
...
[selective_risk/SELECTIVE_RISK_REPORT.md](https://github.com/gasyoun/kosha/blob/main/data/concordance/selective_risk/SELECTIVE_RISK_REPORT.md),
...
gate withheld 266 candidate edges in this run.
