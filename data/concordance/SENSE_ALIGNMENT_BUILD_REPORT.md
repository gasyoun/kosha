...
| PWK (Kielhorn's shorter PW) | no | NOT reachable via csl-sqlite — no `pwk.zip` in the release. The similarly-named `pwkvn.zip` is the Verzeichnis der Namen (24,976 records, inspected 15-09-2026), not the 151,349-key1 dictionary; the encoded source lives in the sanskrit-lexicon/PWK repo and needs its own ingestion slice. Recorded absence (H4745 step 1) rather than a header with nothing under it. |
...
- OUT: PWK — no csl-sqlite source (`pwkvn` is the names index, not the dictionary); recorded absence, its own ingestion slice (H4745 step 1).
...
| senses considered | 36285 |
| senses dropped as `no-gloss` (structural chunks) | 3025 |
| meaning groups | 32867 |
| aligned groups (≥2 dictionaries) | 3045 |
| unaligned singleton senses | 29822 |
| aligned share of groups | 9.3% |
| lemmas with ≥1 aligned group | 483 |
...
| MD senses loaded | 526 |
| aligned groups touching MD | 58 |

## Delta against the H3862 slice-2 baseline (H4745)

Baseline: **H3862 slice 2 (PWG · MW · Apte · ŚKDR · VCP)**, as published in this file on 02-09-2026
...
|---|---:|---:|---:|
| senses considered | 35,759 | 36,285 | +526 |
| aligned groups | 3,013 | 3,045 | +32 |
...
|---|---:|---:|---:|
| senses considered | 33,763 | 36,285 | +2,522 |
| meaning groups (rows) | 30,470 | 32,867 | +2,397 |
| aligned groups | 2,957 | 3,045 | +88 |
| lemmas with ≥1 aligned meaning | 477 | 483 | +6 |
| clean three-dictionary `1-1-1` rows | 262 | 262 | +0 |

| failure class | H3744 | this build | Δ |
|---|---:|---:|---:|
| `no-shared-witness` | 17,671 | 17,968 | +297 |
| `witness-too-common` | 6,844 | 6,929 | +85 |
...
|---|---:|---|
| `no-shared-witness` | 17968 | the sense cites sources, but no cross-dictionary sense of this lemma cites any of them |
| `witness-too-common` | 6929 | shared witnesses exist but fall below τ on weight — real citation, no discriminating power |
