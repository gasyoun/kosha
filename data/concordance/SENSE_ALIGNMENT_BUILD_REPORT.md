...
## H4751 — W2 scaling wave (14-09-2026, landed 15-09)

Slice 2 ran on the frozen 500-headword pilot frame. H4751 scales the SAME aligner,
SAME constants and SAME failure taxonomy onto the 1,500-row W2 frame
(`sense_pilot_headwords_w2.tsv`: the frozen H1670 selection rule — ≥2 leaf senses,
≥2 loci-carrying senses, DCS-attested, rank[attested, n_senses, n_ls] — over the full
PWG sense-loci export instead of the sample; the nāgadanta/nāgadantaka smoke pair stays
forced at the head; 1,487 unique slp1 tokens, 13 homonym pairs). Every number above in
the metric and delta tables is the scaled run; slice-1 comparisons are labelled as such.

| lever | slice 1+2 (frozen pilot) | H4751 W2 scale |
|---|---:|---:|
| frame (unique lemmas) | 500 | 1,487 |
| senses considered | 35,759 | 73,827 |
| meaning groups | 32,399 | 65,987 |
| aligned groups | 3,013 (9.3%) | 7,009 (10.6%) |
| lemmas with ≥1 aligned group | 481 | 1,407 |
| clean western `1-1-1` rows | 262 | 633 |

**Verification (this wave's handoff verify, all machine-checked):**

- **Strata parity** — the frozen H3910 acceptance-strata rule re-derived on the scaled
  table (`sample_sense_alignment_acceptance.py --out-* _w2`): 21 strata, population
  7,009 = aligned-declared (exact), shares sum to 1.0, axis schema (method × score-band ×
  shape), census rule and oversample identical to the frozen H3910 derivation; census
  strata (96 groups) taken whole. New artifacts sit BESIDE the frozen ones
  (`sense_alignment_acceptance_sample_w2.tsv`, `sense_alignment_acceptance_strata_w2.json`);
  the frozen 120-card H3910 sample and strata JSON are untouched (byte-stable in git).
- **Kappa on sample** — `score_sense_alignment_judge.py` re-run over the frozen two-pass
  H3910 verdicts reproduces **Cohen's κ = 0.9022** (raw agreement 95.0%, attrib* channel
  42/42) with byte-identical output artifacts. The scaled table's own precision remains
  unmeasured until a human vote on a W2 sheet — same fence slice 1+2 sat behind; the
  153-card W2 sample above is the sheet's ready feed.
- **Frame canary** — all 1,500 frame rows satisfy the stated criteria (0 violations),
  ordering consistent with the frozen rank (single break = the forced smoke pair),
  and every frame head produced ≥1 output row. `wc -l`: 65,988 alignment rows
  (header + 65,987) and 65,591 failure rows (header + 65,590; class counts match the
  failure table below exactly).

Provenance: `build_sense_alignment.py --heads <1,487 tokens of the W2 frame>` over
kosha.db (sidecar); constants TAU=0.30, GLOSS_FLOOR=0.20, PREFIX_MIN=4 unchanged.
...
| metric | value |
|---|---:|
| lemmas in the run | 1487 |
| senses considered | 73827 |
| senses dropped as `no-gloss` (structural chunks) | 6612 |
| meaning groups | 65987 |
| aligned groups (≥2 dictionaries) | 7009 |
| unaligned singleton senses | 58978 |
| aligned share of groups | 10.6% |
| lemmas with ≥1 aligned group | 1407 |
| lemmas present in all three western dictionaries | 730 |
| ŚKDR senses loaded | 392 |
| VCP senses loaded | 3290 |
| aligned groups touching ŚKDR | 160 |
| aligned groups touching VCP | 7 |
...
|---|---:|---:|---:|
| senses considered | 33,763 | 73,827 | +40,064 |
| meaning groups (rows) | 30,470 | 65,987 | +35,517 |
| aligned groups | 2,957 | 7,009 | +4,052 |
| lemmas with ≥1 aligned meaning | 477 | 1,407 | +930 |
| clean three-dictionary `1-1-1` rows | 262 | 633 | +371 |

| failure class | H3744 | this build | Δ |
|---|---:|---:|---:|
| `no-shared-witness` | 17,671 | 36,059 | +18,388 |
| `witness-too-common` | 6,844 | 12,294 | +5,450 |
| `no-gloss` | 3,025 | 6,612 | +3,587 |
| `cross-language-gap` | 1,835 | 4,315 | +2,480 |
| `no-citation-apparatus` | 0 | 3,262 | +3,262 |
| `outranked` | 1,101 | 2,546 | +1,445 |
| `absent-dictionary` | 62 | 502 | +440 |
...
|---|---:|---|
| `no-shared-witness` | 36059 | the sense cites sources, but no cross-dictionary sense of this lemma cites any of them |
| `witness-too-common` | 12294 | shared witnesses exist but fall below τ on weight — real citation, no discriminating power |
| `no-gloss` | 6612 | a structural chunk (PWG `<div>` carrying only `<lex>m.</lex>`) — excluded before alignment |
| `cross-language-gap` | 4315 | a PWG sense with no `<ls>` at all — the citation bridge does not exist for it, and German→English gloss overlap measures nothing |
| `no-citation-apparatus` | 3262 | a ŚKDR/VCP sense no western sense attributes to its kośa. The kośas carry no `<ls>` at all, and their gloss is Sanskrit, so both bridges are shut for it — a property of the source format, not a tuning failure |
| `outranked` | 2546 | a qualifying partner existed but preferred a better-scoring sense; each sense takes at most one partner per other dictionary |
| `absent-dictionary` | 502 | the lemma simply has no entry in the other dictionaries |
...
[unaligned 0-0-0-0-1 singleton 0.00 -] VCP: nAgadanta pu0 nAgasya hastino danta ivAkAro'styasya ac . gfhaBittirnir  <- no-citation-apparatus
