...
| Loaded rows | **323,422** data rows (file has 323,422 lines incl. header; header row is not data) |
| Deviation from plan | [PHASE1_PLAN.md](https://github.com/gasyoun/kosha/blob/main/PHASE1_PLAN.md) D1 states "323,422 rows" for the `SELECT COUNT(*)` check — that number is off by one against the measured file (323,422 data rows after the header). The correct check value is **323,422**; PHASE1_PLAN's stated figure appears to be a stray off-by-one (possibly counting the header). Recorded here per the anti-gaming rule (measure, don't assume) rather than silently matching the wrong number. |
