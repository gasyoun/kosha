...
COLUMNS = ['lemma_slp1', 'layer', 'sense_id', 'sense_gloss', 'count_all',
           'sense_rank', 'lemma_share', 'periods', 'provenance', 'confidence']
GLOSS_CAP = 100
...
def main():
    ...
    # Rank + share within (lemma, layer).
    by_ll = collections.defaultdict(list)  # (lemma, layer) -> [(sense_id, count, gloss)]
    for (lemma, layer, sid), (c, gloss) in agg.items():
        by_ll[(lemma, layer)].append((sid, c, gloss))
    rows = []
    for (lemma, layer), senses in by_ll.items():
        senses.sort(key=lambda x: (-x[1], x[0]))
        total = sum(c for _, c, _ in senses)
        # dense rank on descending count
        rank, prev, seen = 0, None, 0
        for sid, c, gloss in senses:
            seen += 1
            if c != prev:
                rank = seen
                prev = c
            rows.append({
                ...
                'sense_gloss': gloss, 'count_all': c, 'sense_rank': rank,
