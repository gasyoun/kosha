...
- **Full-data-tier suite repaired on the MSI box: 60 failed → 0 ([#517](https://github.com/gasyoun/kosha/pull/517), OxAlpha `zai-coding-plan/glm-5.3-flash`).** The residual `@DO` from the H4019 census: 60 failures confined to the full-data tier (modules that run only where the 1.7 GB `kosha.db` exists — CI/Mac skip them), pre-existing and orthogonal to the pin work. Three root causes, three fixes. (1) **Salt-contract staleness** (`test_evidence` 24, `test_api` 4, `test_citability` 2): the tests still read pre-W0C shapes — top-level `entry["evidence"]`/`entry["sense_ids"]`/`entry["dict"]` (moved under `entry["kosha"]` in #224) and the `body["detail"]["error"]` envelope (now top-level `body["error"]`); the files only ever ran on full-data boxes, so CI never flagged them. (2) **Feature gate** (`test_history` 11): D10 keeps history/auth/stats unmounted without `KOSHA_HISTORY_ENABLED` (unset on the box), so every route 404'd; the module now uses the documented [kosha.feature_gates.mount_history](https://github.com/gasyoun/kosha/blob/main/src/kosha/feature_gates.py) runtime mount + env flag + `get_settings(refresh=True)` in one autouse fixture instead of a box-local variable. (3) **Test-order isolation** (`test_reverse_lookup` 11, `test_static_cache` 5, remainder in mixed runs): env-patching tests (`test_readiness` et al.) refresh `kosha.settings._cached` under monkeypatch; after teardown the environ is restored but the cache keeps `KOSHA_CORE_DB_PATH` pointing into a deleted `tmp_path`, so later `get_settings()` consumers die with `sqlite3.OperationalError` (proven by pair-run: readiness + reverse_lookup → 9 failed). Fix: autouse [tests/conftest.py](https://github.com/gasyoun/kosha/blob/main/tests/conftest.py) fixture resetting the settings cache around every test. Plus one robustness follow-through: `test_paradigms`/`test_static_cache` resolve the core DB via `get_settings().core_db` (same source of truth as `conftest._core_db_present`) instead of a hardcoded repo-relative path, so the tier also runs from a worktree with `KOSHA_CORE_DB_PATH` set. Verification on the MSI box: **full suite 737 passed, 107 skipped, 0 failed**; fixture tier without the core DB 623 passed (CI parity green); poison repro pair green.
- **H4019 follow-up: UTF-8 opens for the sense-dating build/hydrate ([#516](https://github.com/gasyoun/kosha/pull/516), OxAlpha `zai-coding-plan/glm-5.3-flash`, MSI box).** [scripts/build_sense_dating.py](https://github.com/gasyoun/kosha/blob/main/scripts/build_sense_dating.py) and [app/dating_hydrate.py](https://github.com/gasyoun/kosha/blob/main/app/dating_hydrate.py) read their UTF-8 inputs (`works_hand.tsv`, `abbrev_map.tsv`, `dcs-text-dates-2021.tsv`, `sense_corpus_concordance.tsv`, the two evidence snapshots) with bare `open()`/`read_text()` — locale-encoded. On the MSI box the locale is cp1251, so every non-ASCII work name (ā, ṛ, Devanagari) mangled on read: the hand-verified canon_dm join missed `ramayana` (`--check` exit 1) and the H4026 badge hydrate resolved 0 citations (hits==0). Linux/Mac CI never saw it because their locale is UTF-8. Fix: explicit `encoding="utf-8"` on every read/write in the two files (17 call sites). `tests/test_sense_dating.py` 17/17 green on the MSI box after the fix. Found during the GTD pin-reconcile MSI half (sanskrit-util `e147349`/0.11.0 install + suite run), same row as the 0.117.12 entry.
...
### Added
- **H3549 (OxAlpha) — 30-day risk-ranked code review + inactive OxAlpha status-gate design.** [Evidence report](https://github.com/gasyoun/kosha/blob/main/docs/CODE_REVIEW_KOSHA_OXALPHA_30D_2026-08-26.md): ~100 window PRs (26-07..25-08-2026) censused, 10 executable-risk slices retained with exact base/head SHAs, independent Standards + Spec verdicts (all PASS), 6 findings ledgered (1×P2, 5×P3/informational; **zero proven P0/P1 → zero repair PRs**, recorded explicitly); adapter bootstrap landed separately in #463. [OXALPHA_STATUS_GATE_DESIGN_2026.md](https://github.com/gasyoun/kosha/blob/main/docs/OXALPHA_STATUS_GATE_DESIGN_2026.md): future executable-code review gate + sensitive-path human approval, designed and **not enabled** (no workflow added, protection untouched).
