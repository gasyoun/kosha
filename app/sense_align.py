...
# A German infinitive: lower-case run whose last word ends -en/-ern/-eln. German
# nouns are capitalised, so a noun gloss never matches; an adjective or participle
# in -en (`eigen`, `rothen`, `geworden`) still does — the residual this lens cannot
# see, and it only ever KEEPS an attachment. A `ge-` participle exclusion was tried
# and withdrawn (Astra review of H5273): it also caught `geben`, `gehen`, `gewinnen`
# and gated true verbal matches, the wrong direction to err in.
_DE_INFINITIVE = re.compile(r"^(?:[a-zäöüß]+ )*[a-zäöüß]+(?:en|ern|eln)$")
...
def pwg_pos(body_upto_sense_end: str, gloss: str) -> str:
    ...
    *hinundherbewegen*).
    ...
    first = re.split(r"[;,]", gloss or "", maxsplit=1)[0].strip()   # `halten, tragen` → `halten`
