...
| 20-07-2026 | **H1366 — generated-side decide brief parked + ARCHITECTURE mislabel corrected (did NOT rule).** H1366 was scoped to brief, not rule (the canonical-table choice stays a human call). Measured both tables directly against `kosha.db` — `forms` (1,378,401 / 426,410 non-heritage) and `inflections` (6,917,018, ~100% `cologne_mwinflect`) share only 168k of 426k non-heritage `(form,lemma)` pairs, so they are different data products, not two cardinalities of one. Parked the ruling-ready decide brief ([DECIDE_H1366_GENERATED_SIDE_FORMS_VS_INFLECTIONS.md](https://github.com/gasyoun/kosha/blob/main/docs/DECIDE_H1366_GENERATED_SIDE_FORMS_VS_INFLECTIONS.md)) recommending `forms` at high confidence; added the §2 "not-in-the-settled-table" callout; corrected the ARCHITECTURE §1 node's "6.9M generated" mislabel to "1.38M / 426k non-heritage". No code touched | Opus 4.8 (`claude-opus-4-8`) |
| 20-07-2026 | **H1366 RULED — MG accepted `forms` as canonical.** Added the settled §2 `D13` row + restored §3a (full evidence table + three grounds); flipped the DECIDE brief and ARCHITECTURE §1 note to ✅; CONTRADICTIONS §3 → ✅ tombstone, FINDINGS §94 → ruled. **W2a unblocked to consume `forms`** | Opus 4.8 (`claude-opus-4-8`) |
