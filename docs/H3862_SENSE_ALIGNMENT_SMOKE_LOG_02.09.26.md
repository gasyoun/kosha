...
| every failure row carries a class from the documented taxonomy | ✅ PASS | absent-dictionary, cross-language-gap, no-citation-apparatus, no-gloss, no-shared-witness, outranked, witness-too-common |
| unaligned senses are kept in the table, not dropped | ✅ PASS | 29386 rows |
...
| staged viewer @375px — table has rows | ✅ PASS | 7 rows |
...
| staged viewer @1280px — table has rows | ✅ PASS | 7 rows |
...
python scripts/smoke_sense_alignment.py --log docs/H3862_SENSE_ALIGNMENT_SMOKE_LOG_02.09.26.md
```
