...
_Created: 18-07-2026 · Last updated: 15-09-2026_
...
| **24-07-2026 (close-out)** | **289.7 MiB** | **67.8 MiB** | **0.84 MiB** | **22.4 MiB** | **13.3 MiB** | **130.1 MiB** @ sample mean **11.95 KB/page** (H537 9.7 was inherited) | **523.3 MiB** | **51.1%** | H1586 residual: **N re-measured** from `lemma_frequency.tsv` → still **11,148** / 95.00% (59,282 lemmas · 4,550,704 tokens). Word-page sample render (n=348 head-band cards via `render_word_page`) mean **11.95 KB** vs inherited 9.7 → head +24.5 MiB; still **≪75%** / **≪90%** gates. Full **tracked** tree (incl. `data/`) ≈ **775 MiB** — Pages source is branch root, so non-web data also counts toward soft cap; W5 must keep regenerable bulk gitignored. D4 recorded in root [`ARCHITECTURE.md`](https://github.com/gasyoun/kosha/blob/main/ARCHITECTURE.md). Grok 4.5 (`grok-4.5`) dual-run close-out after #185. |
| **24-07-2026 (W5 / H1590)** | **289.7 MiB** | **67.8 MiB** | **0.84 MiB** | **22.4 MiB** | **13.3 MiB** | **184.9 MB** actual (`docs/w/`, **10,370** pages @ mean **17.4 KB**; D4 N=11,148 / 95.00%; 778 head-without-card → SSR) | **578.1 MB** | **56.5%** | Full static head build via `build_word_pages.py --coverage 0.95`. Mean higher than H1586 sample (multi-dict-heavy head). Still **≪75% / ≪90%**. `docs/w/` + `docs/browse/` gitignored. Exit packet: [`docs/P5_WORD_PAGE_EXIT_PACKET.md`](https://github.com/gasyoun/kosha/blob/main/docs/P5_WORD_PAGE_EXIT_PACKET.md). Grok 4.5 (`grok-4.5`). |
| **30-08-2026** | **303.8 MB** | **76.5 MB** | **0.88 MB** | **25.1 MB** | **13.9 MB** | **234.0 MB** @ sampled mean **21.49 KB/page** (n=300) | **653.2 MB** | **63.8%** | H3745 re-measure with wired 70% (716.8 MB) fail gate — GATE PASS. Cards on disk (input population): 50355. D4 target head N=11148; actual deployed `/w/` page count is a Pages-deploy-side fact (2,324 per H3745 mint, 30-08-2026), not re-measurable from this worktree since `docs/w/` is gitignored. |
| **15-09-2026** | **303.8 MB** | **116.3 MB** | **0.88 MB** | **25.1 MB** | **13.9 MB** | **417.7 MB** committed `w/` byte-true (10370 head pages, 38.48 KB/page) + 0.0 MB unbuilt | **1479.9 MB** | **144.5%** | H4841 recalibrated gate (70% = 716.8 MB, unchanged) — GATE FAIL. Total = whole published tree (Pages source `main:/`) incl. data/ 571.7 MB + other 31.3 MB; served tiers alone 876.9 MB = 85.6%. sampler check +0.37% vs census. D4 N=11148, head with card 10370. |

**W4b conclusion (24-07-2026):** with A4 pages included, the static-head projection stays **well under** the ~90% soft-cap warning threshold (48.8% on H537 KB basis; **51.1%** on re-sampled page sizes). D4 head bound **N=11,148** remains the control after re-measure; no need to re-open N. Live Pages may differ from worktree if deploy omits some of `reading/` or `docs/js` — re-measure on the deploy branch before W5 ships the head. **W5 note:** use measured KB/page at build time (sample now ~12 KB), not the frozen 9.7 figure.

**W5 conclusion (24-07-2026, H1590):** static head **built** (10,370 pages / 184.9 MB / 18.1% of soft cap alone; projected web tier **56.5%**). Agent acceptance 5-1…5-4 met; live Lighthouse / Gītā walkthrough / MG staging sign-off remain **blocked** until deploy (honest ⛔ in exit packet).

**W4b re-measure with a wired fail gate (30-08-2026, H3745):** the prior rows recorded numbers by hand with no threshold that could actually fail — an unattended run would always "succeed". [`scripts/measure_pages_budget.py`](https://github.com/gasyoun/kosha/blob/main/scripts/measure_pages_budget.py) re-measures the D4 head N and mean KB/page at build time (never a carried constant), sums it against the on-disk cards/concordance/reading/docs-js tiers, and **exits 1** if the projected total exceeds **717 MB (70% of the 1,024 MB cap)** — `pytest` fixture-tier CI runs it every build via `tests/test_pages_budget_gate.py`. Today's run: **653.2 MB = 63.8%**, GATE **PASS**. Note the mean KB/page keeps climbing as dictionary coverage per card grows (9.7 → 11.95 → 17.4 → 21.49 KB across four re-measures) — the gate exists precisely because that trend could eventually cross 70% even with N unchanged.

**W4b recalibration (15-09-2026, H4841): the gate was blind on two axes, and now reads GATE FAIL at 144.5%.** (1) The sampler rendered word pages **without** the published ux layer (`DEFAULT_LIVE_UX`, H4026) — 22.80 KB/page vs 36.75 KB on the same 300 cards — and multiplied by N=11,148 instead of the 10,370 head lemmas that have a card. The frame itself already spanned the selection; it is now an explicit stratified frame (take-all top 2% of cards by bytes + size-sorted systematic sample in head/mid/tail rank bands) and lands **+0.37%** from the byte-true census of the committed head (410.1 vs 408.6 MB). (2) The tier list counted neither the committed repo-root `w/` (417.7 MB) nor anything else Pages actually publishes: the Pages source is legacy `main:/`, so the published footprint is the whole tracked tree — **1,479.9 MB**, of which `data/` is 571.7 MB. The gate now measures that tree byte-true by tier (tracked blob sizes at HEAD) plus a projection for any unbuilt head pages. The **70% ceiling is kept** (no threshold drift): served tiers alone are 876.9 MB = 85.6%, the full tree 144.5%, and Pages has still built every deploy at that size. Options and recommendation: [PAGES_HEADROOM_PLAN_H4841_15.09.26.md](https://github.com/gasyoun/kosha/blob/main/docs/PAGES_HEADROOM_PLAN_H4841_15.09.26.md).
